import { CharacterForEscape, assert, hash2String, parseFloat } from "./common";
import { WhiteChars, AllIdentStartChars, AllIdentChars, Digits, HexDigits, BinaryDigits, Symbols, EqualSymbols, CompoundSymbols, Compounds, Keywords, BlockFollowKeyword, UnopSet, BinopSet, UnaryPriority, BinaryPriority } from "./constant";
import { createbinop, createtype, createunop, removeParen, replace } from "./solveMath";
  ...
  formatExpr = function(expr) {
              ...
              || expr.Type == "VargLiteral" || expr.Type == 'HashLiteral')
      {
          ...
          if (expr.Type == 'HashLiteral') {
              expr.Token.Source = '"' + hash2String(`${expr.Token.Source.substring(1, expr.Token.Source.length - 1)}`) + '"'
...
function StripAst(ast) {
  ...
  stripStat = function(stat) {
      if (stat.Type == "StatList") {
          ...
          //console.log(`unreachable, type: ${stat.Type}`,stat)
...
export const Beautify = (scr, options) => {
