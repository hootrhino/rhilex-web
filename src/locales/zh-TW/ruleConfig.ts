export default {
  /**
   * form
   */
  'ruleConfig.form.title.actions': '規則回調',
  'ruleConfig.form.title.sourceType': '數據來源',
  'ruleConfig.form.title.fromSource': '輸入資源',
  'ruleConfig.form.title.testData': '輸入數據',
  'ruleConfig.form.title.output': '輸出結果',
  'ruleConfig.form.title.targetType': '北向資源類型',
  'ruleConfig.form.title.targetId': '北向資源 UUID',
  'ruleConfig.form.title.resourceType': '資源類型',
  'ruleConfig.form.title.resourceId': '資源選擇',
  'ruleConfig.form.title.type': '示例數據結構',

  'ruleConfig.form.placeholder.testData': '請輸入數據',
  'ruleConfig.form.placeholder.targetId': '請選擇資源',

  /**
   * options
   */
  'ruleConfig.type.option.default': '預設數據結構',

  /**
   * title
   */
  'ruleConfig.title': '規則配置',
  'ruleConfig.title.new': '新建規則',
  'ruleConfig.title.edit': '編輯規則',
  'ruleConfig.title.detail': '規則詳情',
  'ruleConfig.title.log': '規則日誌',
  'ruleConfig.title.test': '測試腳本',
  'ruleConfig.title.group.dataTo': '數據推播',
  'ruleConfig.title.group.iothub': '平台系統集成',

  /**
   * button
   */
  'ruleConfig.button.custom': '自訂規則鏈',
  'ruleConfig.button.quick': '快速配置規則',

  /**
   * others
   */
  'ruleConfig.popconfirm.title.remove': '確定要刪除此規則？',
  'ruleConfig.popconfirm.title.reset': '重置可能會丟失數據，確定要重置嗎？',
  'ruleConfig.empty.targetId': '系統未識別到可用資源。請參考操作指南完成資源建置步驟。',
};
