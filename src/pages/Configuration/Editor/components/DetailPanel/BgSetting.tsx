import { getBase64, tryToJSON } from '@/utils/utils';
import { PlusOutlined } from '@ant-design/icons';
import { Graph } from '@antv/x6';
import { Col, ColorPicker, message, Modal, Row, Select, Slider, Space, Upload } from 'antd';
import type { RcFile } from 'antd/es/upload';
import type { UploadFile } from 'antd/es/upload/interface';
import { useEffect, useState } from 'react';
import { POSITION_OPTION, REPEAT_OPTION, SIZE_OPTION } from '../../constants';

type BgSettingsProps = {
...
const BackgroundSettings = ({ onChange }: BgSettingsProps) => {
  const [setting, setSetting] = useState<BgConfig>({
    color: '#F2F7FA',
    showImage: true,
    repeat: 'no-repeat',
    position: 'center',
  ...
  const notifyChange = () => {
    const { showImage, size, position, ...params } = setting;
    onChange({
      ...params,
      image: showImage ? bgImageDataURL : undefined,
      size: tryToJSON(size),
      position: tryToJSON(position),
    });
  };
      ...
      <div className="w-full">
        ...
        <Item label="透明度">
          <Slider
            ...
            onChange={(changedValue: number) => setSetting({ ...setting, opacity: changedValue })}
          />
        </Item>
      </div>
      <div>
        <div className="mb-[10px]">背景图片设置</div>
        <Upload
